<x-app-layout>
                        ...
                        <div class="p-6 bg-white border-b border-gray-200">
                            <div class="p-4">
                                <div class="mb-4 flex justify-between items-center">
                                    <div>
                                        <h2 class="text-xl font-semibold">Karyawan Table</h2>
                                    </div>
                                    ...
                                    <table id="userTable" class="min-w-full bg-white text-sm text-left text-gray-700">
                                        ...
                                        <tbody id="tableBody" class="divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 text-center">{{ $d->identitas }}</td>
                                                <td class="px-6 py-4 text-center">{{ $d->name }}</td>
                                                <td class="px-6 py-4 text-center">{{ $d->hp }}</td>
                                                <td class="px-6 py-4 text-center">{{ $d->jk }}</td>
                                                <td class="px-6 py-4 text-center">{{ $d->alamat }}</td>
                                                <td class="px-6 py-4 text-center">
                                                    <div class="flex justify-center items-center space-x-4">
                                                        <!-- Edit Button -->
                                                        @if(Auth::user()->role === 'keuangan' )
                                                        <a href="{{route('Gaji.karyawan', $d->id)}}" class="text-yellow-500 hover:text-white transition-colors duration-300 p-1 rounded hover:bg-yellow-500">
                                                        ...
                                                        <a href="{{route('Karyawan.edit', $d->id)}}" class="text-blue-500 hover:text-white transition-colors duration-300 p-1 rounded hover:bg-blue-500">
                                            ...
                                            <x-modal name="{{ $d->id ? 'confirm-delete-' . $d->id : null }}" focusable>
                                                <div class="p-8 mx-auto rounded-lg shadow-lg">
                                                    ...
                                                    <p class="text-gray-700">
                                                        Apakah kamu yakin ingin menghapus karyawan <strong class="font-semibold">{{ $d->name }}</strong>?<br>
                                                    ...
                                                    <div class="mt-8 flex justify-end space-x-4">
                                                        ...
                                                        <form action="{{ route('Karyawan.destroy', $d->id) }}" method="POST" class="inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button
                                                                type="submit"
    ...
    <x-modal name="tambah-user" focusable>
        <form method="POST" action="{{ route('Karyawan.store') }}" class="p-6">
            @csrf
            <h2 class="text-lg font-medium text-black">Tambah Karyawan Baru</h2>

            <!-- Nama -->
            <div class="mt-4">
                <x-input-label for="identitas" value="No Identitas" />
                <x-text-input id="identitas" name="identitas" type="text" class="mt-1 block w-full" required autofocus />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="hp" name="hp" type="text" class="mt-1 block w-full" required />
    ...
    <x-modal name="cetak-user" focusable>
        <form method="GET" action="{{route('cetak.karyawan')}}" class="p-6">
